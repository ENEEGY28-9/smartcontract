<script lang="ts">
  ...
  isConnected.subscribe(value => {
    debugInfo.connected = value;
  });

  connectionError.subscribe(value => {
    debugInfo.error = value;
  });

  // Subscribe to game state changes
  gameState.subscribe(state => {
    if (state && state.entities) {
      debugInfo.gameStateReceived = true;
      debugInfo.entityCount = state.entities.length;
  ...
  // Exit function will be passed as prop from parent
  export let exitSpectatorMode = () => {
    // Default fallback - leave spectator mode and return to room browser
    ...
    <div class="debug-panel">
      ...
      <div class="debug-grid">
        <div class="debug-item">
          <label>WebSocket Connected:</label>
          <span class="status {debugInfo.connected ? 'connected' : 'disconnected'}">
            {debugInfo.connected ? '🟢 Yes' : '🔴 No'}
          </span>
        </div>
        <div class="debug-item">
          <label>Game State Received:</label>
          <span class="status {debugInfo.gameStateReceived ? 'connected' : 'disconnected'}">
            {debugInfo.gameStateReceived ? '🟢 Yes' : '🔴 No'}
          </span>
        </div>
        <div class="debug-item">
          <label>Entity Count:</label>
          <span>{debugInfo.entityCount}</span>
        </div>
        <div class="debug-item">
          <label>Last Update:</label>
          <span>{debugInfo.lastUpdate || 'Never'}</span>
        </div>
        {#if debugInfo.error}
