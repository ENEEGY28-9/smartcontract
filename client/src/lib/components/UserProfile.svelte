  ...
  async function saveProfile(event: Event) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Safely parse error response with error handling
        let errorData;
        try {
        ...
        if (errorData.message) {
          if (typeof errorData.message === 'string') {
            ...
            errorMessage = errorData.message.message || errorData.message.error || 'Failed to update profile';
          }
        } else if (errorData.error) {
          if (typeof errorData.error === 'string') {
            errorMessage = errorData.error;
          } else if (typeof errorData.error === 'object') {
            errorMessage = errorData.error.message || errorData.error.error || 'Failed to update profile';
          } else {
