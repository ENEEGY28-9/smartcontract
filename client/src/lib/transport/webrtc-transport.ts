...
import { handleQuantizedSnapshot, handleQuantizedDelta } from './index';
import { defaultWebRTCConfig as baseWebRTCConfig, getOptimalWebRTCConfig } from '../config/webrtc-config';

// Export WebRTCConfig để các file khác có thể sử dụng
...
export class WebRTCTransport extends EventEmitter {
  ...
  send(message: NetworkMessage): void {
    if (!this.isConnected()) {
      console.warn('Cannot send message: WebRTC not connected');
      return;
    }
    ...
    try {
      ...
      if (message.type === 'input' || message.type === 'control') {
        if (this.controlChannel?.readyState === 'open') {
          ...
          console.warn('Control DataChannel not ready');
        }
      } else {
        if (this.stateChannel?.readyState === 'open') {
          ...
          console.warn('State DataChannel not ready');
        }
      }
    } catch (error) {
...
// Default WebRTC configuration với đầy đủ trường cần thiết cho transport
...
// Hàm tạo WebRTC config với TURN servers tối ưu
export async function getEnhancedWebRTCConfig(): Promise<WebRTCConfig> {
  const optimalConfig = await getOptimalWebRTCConfig();
  return {
    ...defaultWebRTCConfig,
    iceServers: optimalConfig.iceServers
  };
}
