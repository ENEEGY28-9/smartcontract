...
mod tests {
    ...
    fn test_snapshot_roundtrip() {
        ...
        if let Some(transform_data) = entity.components.get("transform") {
            if let Ok(transform) = serde_json::from_value::<TransformComponent>(transform_data.clone()) {
                ...
                let (dequantized_pos, _dequantized_rot, _dequantized_scale) = quantized_transform.to_original(&config);
    ...
    fn test_delta_encoding() {
        ...
        let _original_entity = EntitySnapshot {
            id: "test_entity".to_string(),
            components: serde_json::json!({
                "transform": {
    ...
    fn test_basic_quantization_encoding() {
        ...
        // Test that quantized values fit within i16 range
        assert!(quantized_pos.iter().all(|&x| x >= i16::MIN && x <= i16::MAX));
    }
}
