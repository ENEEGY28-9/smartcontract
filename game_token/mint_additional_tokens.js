...
async function mintAdditionalTokens() {
  ...
  // Use the addresses that USER is monitoring (from test_devnet_integration.js)
  ...
  // Mint 100 tokens with 80/20 logic (equivalent to 1 minute of auto-mint scheduler)
  const totalTokensToMint = 100;
  const gameAmount = totalTokensToMint * 0.8; // 80 tokens
  const ownerAmount = totalTokensToMint * 0.2; // 20 tokens
  ...
  try {
    ...
    if (distributionCorrect) {
      ...
      console.log(`• This is equivalent to 1 minute of auto-mint scheduler!`);
    }
