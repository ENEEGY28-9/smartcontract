...
use common_net::{database::{DatabasePool, DatabaseConfig}, shutdown};
...
pub async fn run_with_shutdown(
    ...
    let db_config = DatabaseConfig {
        database_url: std::env::var("DATABASE_URL").unwrap_or_else(|_| "postgresql://postgres:password@localhost/gamev1".to_string()),
        pool_size: 50,           // Max connections in pool
        min_idle: 10,            // Min idle connections
        connection_timeout: 30,  // Connection timeout in seconds
        query_timeout: 10,       // Query timeout in seconds
        enable_metrics: true,
        enable_read_replica: false,
        read_replica_urls: vec![],
    };

    let db_pool = match DatabasePool::new(db_config.clone()).await {
        Ok(pool) => {
            info!("Database connection pool initialized with {} max connections", db_config.pool_size);
            pool
        }
        Err(err) => {
            error!(%err, "Failed to initialize database pool");
            return Err(err);
        }
    };
    ...
    join_set.spawn(async move { worker::run(worker, worker_shutdown, Some(worker_db_pool)).await });
