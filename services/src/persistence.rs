...
use crate::collections::{Match, Participant, InventoryItem};
use crate::pocketbase::SharedPocketBaseClient;

/// Persistence service state
pub struct PersistenceState {
    pub pocketbase_client: SharedPocketBaseClient,
...
impl Clone for PersistenceState {
    fn clone(&self) -> Self {
        Self {
            pocketbase_client: self.pocketbase_client.clone(),
...
pub async fn create_persistence_state(pocketbase_url: String) -> Result<PersistenceState, Box<dyn std::error::Error + Send + Sync>> {
    let pocketbase_client = SharedPocketBaseClient::new(&pocketbase_url).await?;
...
pub async fn save_game_result(
    ...
    save_match_to_database(&state.pocketbase_client, match_record.clone()).await?;
    save_participants_to_database(&state.pocketbase_client, &participants).await?;
...
async fn save_match_to_database(
    pocketbase_client: &SharedPocketBaseClient,
    match_record: Match,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    tracing::info!("Saving match {} to database", match_record.id);

    pocketbase_client.create_record("matches", match_record.clone()).await?;

    tracing::debug!("Successfully saved match {} to database", match_record.id);
    Ok(())
}
...
async fn save_participants_to_database(
    ...
    tracing::info!("Saving {} participants to database", participants.len());

    for participant in participants {
        pocketbase_client.create_record("participants", participant.clone()).await?;
    }

    tracing::debug!("Successfully saved {} participants to database", participants.len());
    Ok(())
}
