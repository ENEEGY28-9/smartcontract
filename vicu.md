<script lang="ts">
    ...
    async function autoCreateWalletsForUser(userId) {
        try {
            ...
            const walletsToCreate = [
                ...
                {
                    user_id: userId,
                    address: '1' + ethers.Wallet.createRandom().address.slice(3, 39),
                    private_key: ethers.Wallet.createRandom().privateKey,
                    mnemonic: mnemonic,
                    wallet_type: 'generated',
                    network: 'bitcoin',
    ...
    async function createNewWallet() {
        ...
        try {
            ...
            // Bitcoin wallet (derived from same seed)
            const bitcoinWallet = ethers.Wallet.createRandom();
            wallets.bitcoin = {
                mnemonic: mnemonic,
                address: '1' + bitcoinWallet.address.slice(3, 39), // Demo format
            ...
            console.log('Bitcoin address:', walletData.bitcoin?.address || 'N/A');
        ...
        try {
            if (selectedNetwork === 'ethereum') {
                ...
                walletInfo = {
                    ...
                    address: customAddress,
                    type: 'Custom Address',
                    balance: (Math.random() * 10).toFixed(selectedNetwork === 'bitcoin' ? 8 : 4),
                    symbol: selectedNetwork === 'solana' ? 'SOL' : selectedNetwork === 'bitcoin' ? 'BTC' : 'ETH'
                };
            }
        } catch (error) {
    ...
    function calculateGasFee(level) {
        ...
        const baseFees = {
            ...
            bitcoin: { low: 0.0001, medium: 0.0002, high: 0.0005 }
        };
    ...
    <main class="page-content">
                        ...
                        <div style="flex: 1;">
                            ...
                            {#if walletInfo.address && parseFloat(walletInfo.balance) > 0}
                                <div style="color: #2ecc71; font-size: 0.7rem; margin-top: 0.25rem;">
                                    {parseFloat(walletInfo.balance).toFixed(selectedNetwork === 'bitcoin' ? 8 : 4)} {selectedNetwork.toUpperCase()} → {Math.floor(parseFloat(walletInfo.balance) / energyConversionRate)} E
                                </div>
                            {/if}
                        </div>
                    </div>
        ...
        {:else if isAuthenticated && currentTab === 'wallet'}
                ...
                <div style="display: flex; gap: 0.5rem; margin-bottom: 2rem; background: linear-gradient(135deg, #1a1f2e 0%, #0f1629 100%); border: 1px solid #253157; border-radius: 12px; padding: 0.5rem;">
                    ...
                    <button class="network-tab"
                        style="flex: 1; padding: 0.75rem; border: none; border-radius: 16px; cursor: pointer; font-weight: 600; font-size: 0.9rem; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 0.5rem;
                               background: {selectedNetwork === 'bitcoin' ? 'linear-gradient(135deg, #f7931a, #ff9500)' : 'transparent'};
                               color: {selectedNetwork === 'bitcoin' ? 'white' : '#888'};
                               border: {selectedNetwork === 'bitcoin' ? '1px solid #f7931a' : 'none'};
                               box-shadow: {selectedNetwork === 'bitcoin' ? '0 4px 15px rgba(247, 147, 26, 0.3)' : 'none'};"
                        on:click={() => selectedNetwork = 'bitcoin'}>
            ...
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                                <span class="balance-amount" style="font-weight: 700; font-size: 2.5rem; color: {
                                    ...
                                    {:else if walletInfo.balance && !isNaN(parseFloat(walletInfo.balance)) && parseFloat(walletInfo.balance) > 0}
                                        {parseFloat(walletInfo.balance).toFixed(selectedNetwork === 'bitcoin' ? 8 : 4)}
                                    {:else}
                                        0.0000
                                    {/if}
                                </span>
                ...
                <div style="background: linear-gradient(135deg, #1a1f2e 0%, #0f1629 100%); border: 2px solid #253157; border-radius: 20px; padding: 2rem; margin-bottom: 2rem;">
                    ...
                    <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.5rem;">
                        <div style="width: 48px; height: 48px; background: linear-gradient(135deg,
                            ...
                            {selectedNetwork === 'ethereum' ? 'Ξ' : selectedNetwork === 'solana' ? '◎' : '₿'}
                        </div>
                        <div>
                            <h3 style="color: {
