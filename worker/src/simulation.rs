...
use crate::client_prediction::ClientPredictionEngine;
...
impl DeltaEncoder {
    ...
    fn create_delta(&self, current: &QuantizedSnapshot, previous: &QuantizedSnapshot, _current_tick: u64) -> DeltaSnapshot {
    ...
    fn has_significant_change(&self, current: &QuantizedEntitySnapshot, previous: &QuantizedEntitySnapshot) -> bool {
        // Check position change với safe subtraction để tránh overflow
        let pos_diff_x = current.transform.position.0.abs_diff(previous.transform.position.0) > 1;
        let pos_diff_y = current.transform.position.1.abs_diff(previous.transform.position.1) > 1;
        let pos_diff_z = current.transform.position.2.abs_diff(previous.transform.position.2) > 1;

        // Check velocity change (nếu có)
        let vel_changed = match (&current.velocity, &previous.velocity) {
            (Some(curr_vel), Some(prev_vel)) => {
                curr_vel.velocity.0.abs_diff(prev_vel.velocity.0) > 2 ||
                curr_vel.velocity.1.abs_diff(prev_vel.velocity.1) > 2 ||
                curr_vel.velocity.2.abs_diff(prev_vel.velocity.2) > 2
            }
...
impl SimulationWorld {
    ...
    pub fn add_input(&mut self, input: PlayerInput) {
        ...
        let insert_pos = self.input_buffer.inputs.partition_point(|i| i.input_sequence < input.input_sequence);
        self.input_buffer.inputs.insert(insert_pos, input);
    }

    pub fn get_pending_inputs(&self) -> Vec<&PlayerInput> {
        self.input_buffer.inputs.iter().collect()
    }

    pub fn mark_processed(&mut self, sequence: u32) {
        self.input_buffer.last_processed_sequence = sequence;
        // Remove các input đã xử lý
        self.input_buffer.inputs.retain(|input| input.input_sequence > self.input_buffer.last_processed_sequence);
    }
...
pub struct GameWorld {
    ...
    pub state_sync: StateSyncManager, // State synchronization framework
    pub client_prediction: ClientPredictionEngine, // Client prediction system
}
...
impl GameWorld {
    pub fn new() -> Self {
        ...
        Self {
            ...
            current_tick: 0,
            state_sync: StateSyncManager::new(3, 60), // 3 tick delay, 60s full update interval
            client_prediction: ClientPredictionEngine::new(),
        }
    }
    ...
    pub fn tick(&mut self) -> EncodedSnapshot {
        ...
        // Add to state sync history
        let processed_inputs = self.collect_processed_inputs();
        self.state_sync.framework.add_server_state(base_snapshot.clone(), processed_inputs);
    ...
    pub fn run_simulation_for_test(&mut self, duration_secs: f32) -> Vec<EncodedSnapshot> {
        ...
        self.accumulator = self.tick_rate * target_ticks;
    ...
    pub fn force_keyframe_for_player(&mut self, _player_id: &str) -> EncodedSnapshot {
    ...
    pub fn get_snapshot_for_player(&mut self, player_id: &str) -> EncodedSnapshot {
        ...
        let aoi_entities = if let Some(_player_aoi) = self.player_aois.get(player_id) {
        ...
        // Use state sync để generate optimized update
        let current_tick = self.world.resource::<TickCount>().0;

        // Check if cần reconciliation hoặc special handling
        if self.state_sync.framework.needs_reconciliation(player_id, current_tick) {
            // Generate reconciliation message
            if let Some(reconciliation) = self.state_sync.framework.calculate_reconciliation(player_id, current_tick) {
                // Apply reconciliation phía server
                self.client_prediction.apply_reconciliation(player_id, &reconciliation);
            }
        }
    ...
    pub fn get_spectator_snapshots(&mut self) -> Vec<SpectatorSnapshot> {
        ...
        for (_entity, spectator, transform) in query.iter(&self.world) {
    ...
    fn ingest_inputs(&mut self) {
        ...
        for (player_id, buffer) in &mut self.input_buffers {
            let pending_inputs = &buffer.inputs;
    ...
    fn gameplay_logic(&mut self) {
        ...
        // 1. Player vs Pickups
        {
            ...
            for (_player_entity, player_transform, player, _player_rigid_body) in player_query.iter(&self.world) {
                for (pickup_entity, pickup_transform, pickup, _pickup_rigid_body) in pickup_query.iter(&self.world) {
        ...
        // 3. Player vs Enemies (combat damage)
        {
            ...
            for (_player_entity, player_transform, player, _player_rigid_body) in player_query.iter(&self.world) {
                for (_enemy_entity, enemy_transform, enemy, _enemy_rigid_body) in enemy_query.iter(&self.world) {
                    ...
                    if distance < 1.0 && enemy.last_attack.elapsed() >= enemy.attack_cooldown {
        ...
        // 4. Player vs Obstacles (đơn giản là không thể đi qua) - collect changes first
        {
            ...
            for (player_entity, player_transform, _player_rigid_body) in player_query.iter(&self.world) {
                for (_obstacle_entity, obstacle_transform, _obstacle, _obstacle_rigid_body) in obstacle_query.iter(&self.world) {
                    let player_pos = vector![player_transform.position[0], player_transform.position[1], player_transform.position[2]];
        ...
        for (player_id, damage) in damage_to_players {
            if let Some(player_entity) = self.world.resource::<PlayerEntityMap>().map.get(&player_id) {
                if let Some(_player) = self.world.get_mut::<Player>(*player_entity) {
    ...
    /// Collect processed inputs trong tick hiện tại
    pub fn collect_processed_inputs(&mut self) -> Vec<PlayerInput> {
        let mut processed_inputs = Vec::new();

        // Collect inputs từ tất cả players đã được xử lý
        for (_player_id, buffer) in &self.input_buffers {
    ...
    fn cleanup(&mut self) {
        ...
        // Cleanup state sync và client prediction systems
        self.state_sync.cleanup();
    }
    ...
    pub fn add_player(&mut self, player_id: String) -> Entity {
        ...
        if let Some(mut player_map) = self.world.get_resource_mut::<PlayerEntityMap>() {
            player_map.map.insert(player_id.clone(), entity_id);
        }

        // Add to spatial grid
        self.spatial_grid.add_entity(entity_id, [0.0, 5.0, 0.0]);

        // Initialize client prediction cho player mới
        let snapshot = self.create_snapshot();
        self.client_prediction.initialize_player(player_id.clone(), &snapshot);
